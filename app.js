...
app.get('/api/about', (req, res) => {
  fs.readFile(path.join(__dirname, 'public', 'about.json'), 'utf8', (err, data) => {
    if (err) return res.json({ overview: '' });
    try {
      const about = JSON.parse(data);
      res.json({ overview: about.overview || '' });
    } catch {
      res.json({ overview: '' });
    }
  });
...
app.post('/api/about', authenticateAdmin, (req, res) => {
  const { overview } = req.body;
  fs.writeFile(
    path.join(__dirname, 'public', 'about.json'),
